...
def get_best_move(player, board, window_size):
    ...
    for col in range(len(board[0])):
        if is_valid_move(board, col):
            ...
            if player == 'X':
                eval_val = minimax(new_board, False, 'X', 'O', window_size, 0, alpha, beta)
            else:
                eval_val = minimax(new_board, True, 'X', 'O', window_size, 0, alpha, beta)
            undo_move(board, col)
            if player == 'X':
...
def minimax(board, maximizing_player, max_player, min_player, window_size, depth_count, alpha, beta):
